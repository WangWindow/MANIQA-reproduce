...
if __name__ == "__main__":
    ...
    if config.dataset_name == "kadid10k":
        from utils.kadid10k.kadid10k import Kadid10k

        train_name, val_name = split_dataset_kadid10k(
            txt_file_name=config.kadid10k_label, split_seed=config.split_seed
        )
        ...
        from utils.PIPAL22.pipal import PIPAL
        ...
        from utils.koniq10k.koniq10k import Koniq10k

        train_name, val_name = split_dataset_koniq10k(
            txt_file_name=config.koniq10k_label, split_seed=config.split_seed
        )
    ...
    logging.info(
        "{} : {} [M]".format(
            "#Params", sum(map(lambda x: x.numel(), net.parameters())) / 10**6
        )
    )
    ...
    for epoch in range(0, config.n_epoch):
        ...
        if (epoch + 1) % config.val_freq == 0:
            ...
            if rho_s + rho_p > main_score:
                ...
                logging.info(
                    "Saving weights and model of epoch{}, SRCC:{}, PLCC:{}".format(
                        epoch + 1, best_srocc, best_plcc
                    )
                )
